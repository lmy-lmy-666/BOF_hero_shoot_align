...
PoseBridgeNode::PoseBridgeNode(const rclcpp::NodeOptions &options) {
  ...
  set_pose_sub_ =
      node_->create_subscription<geometry_msgs::msg::TransformStamped>(
          "/referee_system/ign/set_pose", 10,
          std::bind(&PoseBridgeNode::set_pose_cb, this, _1));
}
...
void PoseBridgeNode::set_pose_cb(
  ...
  if (executed) {
    if (!result) {
      RCLCPP_ERROR(node_->get_logger(), "Ignition Service %s call failed\n %s",
                   gz_service_name_.c_str(), req.DebugString().c_str());
    }
  } else {
    RCLCPP_ERROR(node_->get_logger(), "Ignition Service %s call timed out\n %s",
                 gz_service_name_.c_str(), req.DebugString().c_str());
  }
}

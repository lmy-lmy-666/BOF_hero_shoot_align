...
struct VisualEntityInfo {
  ...
  int state;
  VisualEntityInfo(Entity _entity, Entity parentEntity, sdf::Visual &_visualSdf,
                   int _state)
      : entity(_entity), parentEntity(parentEntity), visualSdf(_visualSdf),
        state(_state) {}
};
...
LightBarController::LightBarController()
    : dataPtr(std::make_unique<LightBarControllerPrivate>()) {}

void LightBarController::Configure(
    const Entity &_entity, const std::shared_ptr<const sdf::Element> &_sdf,
    EntityComponentManager &_ecm, EventManager &_eventMgr) {
  this->dataPtr->model = Model(_entity);
  if (!this->dataPtr->model.Valid(_ecm)) {
    gzerr << "LightBarController plugin should be attached to a model entity. "
  ...
  if (_sdf->HasElement("initial_color")) {
    ...
    if (color_map.find(color) != color_map.end()) {
      ...
      gzwarn << "LightBarController color [" << color << "] is invalid."
             << std::endl;
    }
  }
  // link_visual
  ...
  // Model Sdf
  this->dataPtr->modelSdf =
      _ecm.Component<components::ModelSdf>(_entity)->Data();
  // Subscribe to commands
  std::string topic{this->dataPtr->model.Name(_ecm) + "/" + controller_name +
                    "/set_state"};
  this->dataPtr->node.Subscribe(topic, &LightBarControllerPrivate::OnCmd,
                                this->dataPtr.get());
  gzmsg << "LightBarController subscribing to int32 messages on [" << topic
        << "]" << std::endl;
}
...
void LightBarControllerPrivate::Init(gz::sim::EntityComponentManager &_ecm) {
  ...
  for (auto linkVisual : this->linkVisuals) {
    ...
    if (v.size() == 2) {
      ...
      auto visual =
          _ecm.EntityByComponents(components::ParentEntity(link),
                                  components::Name(v[1]), components::Visual());
      if (visual != kNullEntity) {
        sdf::Visual visualSdf =
            *(this->modelSdf.LinkByName(v[0])->VisualByName(v[1]));
        this->visualEntityInfos.emplace_back(visual, link, visualSdf, 2);
        flag = true;
      }
    }
    if (!flag) {
      gzerr << "LightBarController: visual element of link [" << linkVisual
            << "] is invaild" << std::endl;
    }
  }
}
...
/******************register*************************************************/
GZ_ADD_PLUGIN(LightBarController, gz::sim::System,

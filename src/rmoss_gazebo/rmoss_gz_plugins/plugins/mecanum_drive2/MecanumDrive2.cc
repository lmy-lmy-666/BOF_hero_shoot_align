...
const std::string kSdfElemJointNames[WHEEL_NUM] = {
    "front_right_joint", "front_left_joint", "rear_right_joint",
    "rear_left_joint"};

class gz::sim::systems::MecanumDrive2Private {
public:
  // Indicates joint/link of which wheel
  enum { FRONT_RIGHT = 0, FRONT_LEFT = 1, REAR_RIGHT = 2, REAR_LEFT = 3 };

public:
  void OnCmdVel(const gz::msgs::Twist &_msg);

  void UpdateOdometry(const gz::sim::UpdateInfo &_info,
...
MecanumDrive2::MecanumDrive2()
    : dataPtr(std::make_unique<MecanumDrive2Private>()) {}
...
void MecanumDrive2::Configure(const Entity &_entity,
  ...
  if (!this->dataPtr->model.Valid(_ecm)) {
    gzerr << "MecanumDrive2 plugin should be attached to a model entity. "
  ...
  this->dataPtr->chassisLink =
      this->dataPtr->model.LinkByName(_ecm, this->dataPtr->chassisLinkName);
  if (this->dataPtr->chassisLink == kNullEntity) {
    gzerr << "chassis link with name[" << this->dataPtr->chassisLinkName
          << "] not found. " << std::endl;
    return;
  }
  // Get joints and links of wheel
  for (int i = 0; i < WHEEL_NUM; i++) {
    this->dataPtr->wheelJointNames[i] =
        _sdf->Get<std::string>(kSdfElemJointNames[i]);
    this->dataPtr->wheelJoints[i] = this->dataPtr->model.JointByName(
        _ecm, this->dataPtr->wheelJointNames[i]);
    if (this->dataPtr->wheelJoints[i] == kNullEntity) {
      gzerr << "wheel joint with name[" << this->dataPtr->wheelJointNames[i]
            << "] not found. " << std::endl;
      return;
    }
  }
  ...
  this->dataPtr->node.Subscribe(topic, &MecanumDrive2Private::OnCmdVel,
                                this->dataPtr.get());
  gzmsg << "MecanumDrive2 subscribing to twist messages on [" << topic << "]"
        << std::endl;
  ...
  this->dataPtr->odomChildFrameId =
      this->dataPtr->model.Name(_ecm) + "/" +
      gz::common::replaceAll(this->dataPtr->chassisLinkName, "::", "/");
  // init PID
...
void MecanumDrive2::PreUpdate(const gz::sim::UpdateInfo &_info,
  ...
  if (!_ecm.Component<components::LinearVelocity>(this->dataPtr->chassisLink)) {
    _ecm.CreateComponent(this->dataPtr->chassisLink,
  ...
  // current state
  const auto chassisPose =
      _ecm.Component<components::WorldPose>(this->dataPtr->chassisLink)->Data();
  const auto linearVel =
      _ecm.Component<components::LinearVelocity>(this->dataPtr->chassisLink)
          ->Data();
  const auto angularVel =
      _ecm.Component<components::AngularVelocity>(this->dataPtr->chassisLink)
          ->Data();
  ...
  // gzmsg << "MecanumDrive2 (force,torque):[" << force << "], [" << torque <<
  // "]" << std::endl; Apply the wrench
  chassisLink.AddWorldWrench(_ecm, force, torque);
}
void MecanumDrive2::PostUpdate(const gz::sim::UpdateInfo &_info,
                               const gz::sim::EntityComponentManager &_ecm) {

  // 1.check collsion  of wheel's link and set the wheel's state true if the
  // wheel contacts with ground plane, and then can compute force and torque
  // based wheel states. (TODO) 2.for odometer
...
void MecanumDrive2Private::OnCmdVel(const gz::msgs::Twist &_msg) {
  ...
  this->targetVel = _msg;
  // gzmsg << "MecanumDrive2 msg x: [" << _msg.linear().x() << "]" <<
  // std::endl;
}

void MecanumDrive2Private::UpdateOdometry(
    const gz::sim::UpdateInfo &_info,
    const gz::sim::EntityComponentManager &_ecm) {
  // get pose and velocity of chassis
  Link chassisLink(this->chassisLink);
  const auto chassisPose =
      _ecm.Component<components::WorldPose>(this->chassisLink)->Data();
  const auto linearVel =
      _ecm.Component<components::LinearVelocity>(this->chassisLink)->Data();
  const auto angularVel =
      _ecm.Component<components::AngularVelocity>(this->chassisLink)->Data();
...
/******************register*************************************************/
GZ_ADD_PLUGIN(MecanumDrive2, gz::sim::System, MecanumDrive2::ISystemConfigure,
              MecanumDrive2::ISystemPreUpdate, MecanumDrive2::ISystemPostUpdate)
